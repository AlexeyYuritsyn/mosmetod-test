...
class myTinymce extends InputWidget
{
     ...
     * @return string
     * */
    public function setOptionsContentWidget($instance)
    {
        //set up filemanager path
        if (!ArrayHelper::getValue($this->pluginOptions, 'external_filemanager_path')) {
            $this->pluginOptions['external_filemanager_path'] = $instance->baseUrl . '/filemanager/';
        ...
        $this->pluginOptions['filemanager_access_key'] = urlencode(serialize($configPath));
//        $id = $this->options['id'];
        return Json::encode(ArrayHelper::merge(['selector' => '.content-widget'], $this->pluginOptions)); //
    }
    ...
    public function registerAssets()
    {
        ...
        if(isset($this->pluginOptions['display_other_forms']) && $this->pluginOptions['display_other_forms'] == true)
        {
            $optionsContentWidget = $this->setOptionsContentWidget($instance);
            $view->registerJs("setInterval(function() { tinymce.init($optionsContentWidget); $('.tox-tinymce').css({'min-height' : '300px'});}, 700); ");
        }
    }
