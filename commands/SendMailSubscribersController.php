 ...
 * @since 2.0
 */
class SendMailSubscribersController extends Controller
{
    public function actionIndex()
    {
        $start_date = strtotime(date("d.m.Y 00:00:00", strtotime("-1 DAY")));
        $end_date = strtotime(date("d.m.Y 23:59:59", strtotime("-1 DAY")));
...
//        echo date("d M Y H:i:s", strtotime("-1 DAY"))."\n";
        ...
        if(!empty($materials))
        {
            $subscribers = \app\models\Subscribers::find()->where(['status' => Subscribers::CONFIRMATION,'email'=>'i25585@yandex.ru'])->all();
...
//                            $end_value_array_parent_categories = end($parent_categories);
